      ...
      <View style={{ flex: 1, backgroundColor: 'rgba(31,32,48,0.5)', justifyContent: 'flex-end' }}>
        <View style={{ backgroundColor: colors.card, borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: '88%' }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', paddingHorizontal: 22, paddingBottom: 4 }}>
            <Text style={{ fontSize: 17, fontFamily: 'SpaceGrotesk_700Bold', color: colors.text, flex: 1 }} numberOfLines={2}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 22, paddingBottom: 10, gap: 12 }}>
            {meeting.startedAt ? (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                ...
                <Text style={{ fontSize: 11, color: colors.textSoft, fontFamily: 'Poppins_400Regular' }}>
            ...
            {loading && (
              <Text style={{ fontSize: 11, color: colors.textSoft, fontFamily: 'Poppins_400Regular' }}>Loading…</Text>
            )}
          </View>
          ...
          {Array.isArray(meeting.attendees) && meeting.attendees.length > 0 && (
            <View style={{ paddingHorizontal: 22, paddingBottom: 10 }}>
              <Text style={{ fontSize: 10, color: colors.textSoft, fontFamily: 'Poppins_700Bold', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Attendees</Text>
                ...
                {meeting.attendees.map((a: any, i: number) => {
                  ...
                  return (
                    <View key={i} style={{ alignItems: 'center', marginRight: 12 }}>
                      <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: colors.purpleSoft, alignItems: 'center', justifyContent: 'center', marginBottom: 3 }}>
                        {a?.avatar ? (
                      ...
                      <Text style={{ fontSize: 9, color: colors.text, fontFamily: 'Poppins_600SemiBold', maxWidth: 50, textAlign: 'center' }} numberOfLines={1}>
                        {name.split(' ')[0]}
                      </Text>
                    </View>
                  );
          ...
          {/* Tab bar */}
          <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: colors.border, paddingHorizontal: 22 }}>
            {(['summary', 'actions', 'transcript'] as const).map((t) => {
              ...
              return (
                ...
                >
                  <Text style={{ fontSize: 12, fontFamily: active ? 'Poppins_700Bold' : 'Poppins_600SemiBold', color: active ? '#6c5ce7' : colors.textSoft }}>
                    {label}
                  </Text>
                </TouchableOpacity>
              );
          ...
          <ScrollView style={{ padding: 22 }} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
              ...
              <>
                {meeting.agenda ? (
                  <View style={{ marginBottom: 16, padding: 12, backgroundColor: 'rgba(108,92,231,0.06)', borderRadius: 14, borderWidth: 1, borderColor: 'rgba(108,92,231,0.15)' }}>
                    ...
                    <Text style={{ fontSize: 12.5, color: colors.text, lineHeight: 19, fontFamily: 'Poppins_400Regular' }}>{meeting.agenda}</Text>
                  </View>
                ) : null}
                <Text style={{ fontSize: 13, color: colors.text, lineHeight: 20, fontFamily: 'Poppins_400Regular' }}>
                ...
                {meeting.carriedOverActionItems.map((ai: any, i: number) => (
                  <View key={`carry-${i}`} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 8, padding: 12, backgroundColor: 'rgba(245,158,11,0.06)', borderRadius: 14, borderWidth: 1, borderColor: 'rgba(245,158,11,0.2)' }}>
                    ...
                    <View style={{ flex: 1, marginLeft: 8 }}>
                      <Text style={{ fontSize: 13, fontFamily: 'Poppins_600SemiBold', color: colors.text }}>{ai.text || String(ai)}</Text>
                      ...
                      {ai.fromMeeting?.title ? (
                        <Text style={{ fontSize: 9.5, color: colors.textSoft, fontFamily: 'Poppins_400Regular', marginTop: 2 }}>
            ...
            {tab === 'actions' && (
              hasActions ? meeting.actionItems.map((ai: any, i: number) => (
                <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 10, padding: 12, backgroundColor: colors.purpleSoft, borderRadius: 14, borderWidth: 1, borderColor: colors.border }}>
                  <Check size={14} color={ai.status === 'done' ? '#10b981' : colors.textSoft} style={{ marginTop: 2 }} />
                  <View style={{ flex: 1, marginLeft: 8 }}>
                    <Text style={{ fontSize: 13, fontFamily: 'Poppins_600SemiBold', color: colors.text }}>{ai.text || ai.title || String(ai)}</Text>
                    {(ai.assignedToName || ai.assignedTo?.full_name) && (
                ...
                <Text style={{ fontSize: 12, color: colors.textSoft, fontFamily: 'Poppins_400Regular', textAlign: 'center', marginTop: 24 }}>
              ...
              <>
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: emailLoading ? colors.purpleSoft : '#6c5ce7',
                  ...
                  Array.isArray(meeting.transcriptChunks) && meeting.transcriptChunks.length > 0
                    ? meeting.transcriptChunks.map((c: any, i: number) => (
                        <View key={i} style={{ marginBottom: 8 }}>
                          ...
                          <Text style={{ fontSize: 13, color: colors.text, lineHeight: 20, fontFamily: 'Poppins_400Regular' }}>{c.text}</Text>
                        </View>
                      ))
                    : <Text style={{ fontSize: 13, color: colors.text, lineHeight: 20, fontFamily: 'Poppins_400Regular' }}>{meeting.transcriptRaw}</Text>
                ) : (
                  <Text style={{ fontSize: 12, color: colors.textSoft, fontFamily: 'Poppins_400Regular', textAlign: 'center', marginTop: 24 }}>
