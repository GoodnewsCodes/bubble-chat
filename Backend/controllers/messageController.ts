...
export const formatMessage = async (m: any) => {
  const rawFileName = m.fileName
    || m.media_metadata?.file_name
    || m.media_metadata?.original_name
    || (m.mediaUrl ? (m.mediaUrl.split('?')[0].split('#')[0].split('/').pop() || '')
      .replace(/^messages_[a-f0-9]+_\d+-[a-z0-9]+-/i, '')
      .replace(/^\d+-[a-z0-9]+-/i, '') : null);
  ...
  return {
    ...
    media_metadata: {
      ...(m.media_metadata || {}),
      ...(rawFileName ? { file_name: rawFileName } : {}),
    },
...
export const emitToConversation = async (
  ...
  export const sendMessage = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    if (req.file) {
      try {
        ...
        const safeOrigName = origName.replace(/[^a-zA-Z0-9.-]/g, '_');
        ...
        if (message_type === 'voice' || req.file.mimetype.startsWith('audio/')) {
          try {
            const ext = path.extname(origName) || '.webm';
    ...
    try {
      ...
      const newMessage = await Message.create({
        ...
        parent_message,
        fileSize,
        fileName: media_metadata?.file_name || undefined,
        media_metadata,
  ...
  /**
   * Proxy media requests to get signed URLs or download files with correct attachment filename
   */
  export const proxyMedia = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      await streamS3Object(url as string, res, filename as string | undefined, req.headers.range as string | undefined);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  };
